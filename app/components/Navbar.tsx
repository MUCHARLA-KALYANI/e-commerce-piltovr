...
const Navbar = (props: Props) => {
    ...
    const SignOut = () => {
        if(session && session.user){
            return(
                <ul style={{  justifyContent: 'flex-start', marginLeft:'-9px' }} className='py-5 px-1 text-neutral-600'>
                    <li  className=' hover:text-blue-500 px-5 py-2 cursor-pointer'>{session.user.name}</li>
                    <li  onClick={() => signOut()} className='whitespace-nowrap hover:text-red-600 px-5 py-2 cursor-pointer'>SignOut</li>
                    <li  className='whitespace-nowrap hover:text-blue-500 px-5 py-2 cursor-pointer'><a href="/addproduct">Add Product</a></li>
                </ul>
            )
        }
        return(
        ...
        <div className='flex items-center justify-between py-4 relative'>
            ...
            <div className='flex items-center space-x-4'>
                ...
                <div onClick={() => setShowProfile(!showProfile)} className='relative cursor-pointer'>
                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTkGse01JRgQM7f56Z8NlULlkrR--hUuyNNug&s" className='w-[35px] h-[35px] rounded-full object-cover' alt="" />
